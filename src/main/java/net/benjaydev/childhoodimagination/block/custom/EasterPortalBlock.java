...
import net.benjaydev.childhoodimagination.worldgen.portal.ModTeleporter;
    ...
    private void handlePortal(Entity player, BlockPos pPos) {
        if (player.level() instanceof ServerLevel serverlevel) {
            ...
            ResourceKey<Level> resourcekey = player.level().dimension() == ModDimensions.EASTER_LAND_LEVEL_KEY ?
                    Level.OVERWORLD : ModDimensions.EASTER_LAND_LEVEL_KEY;

            ServerLevel portalDimension = minecraftserver.getLevel(resourcekey);
            if (portalDimension != null && !player.isPassenger()) {
                if(resourcekey == ModDimensions.EASTER_LAND_LEVEL_KEY) {
