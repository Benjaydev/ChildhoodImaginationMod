...
public class PurpleEasterChickenEntity extends EasterChickenEntity{
    public PurpleEasterChickenEntity(EntityType<? extends EasterChickenEntity> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
    }

    @Override
    public RegistryObject<Item> GetEggDrop() {
        return ModItems.PURPLE_EASTER_EGG;
    }

    @Nullable
    @Override
    public EasterChickenEntity getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        return ModEntities.PURPLE_EASTER_CHICKEN.get().create(pLevel);
    }
}
